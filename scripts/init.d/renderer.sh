...
running() {
  runuser -l "$USER" -c "$DAEMON list 2>/dev/null | grep ${APP} 2>&1 >/dev/null"
    return $?
}

start_server() {
  runuser -l "$USER" -c "$DAEMON start -c coffee ${APP} 2>&1 >/dev/null"
  return $?
}

stop_server() {
  runuser -l "$USER" -c "$DAEMON stop -c coffee ${APP} 2>&1 >/dev/null"
  return $?
}
